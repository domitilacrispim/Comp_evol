...
individuo avalia_r2_final_final (individuo id){
	int reticulo[149], evol[149], notas=0, aux, zeros;	
	for( int i=0; i<10000; i++){
		for ( int ri=0; ri<149; ri++){
				reticulo[ri]=rand()%2;
...
individuo avalia_r3 (individuo id){
	int reticulo[149], evol[149], notas=0, aux, zeros;	
	for( int i=0; i<100; i++){
		for ( int ri=0; ri<149; ri++){
				reticulo[ri]=reticulados[i][ri];
		...
		for(int ger=0; ger<300; ger++){
			for ( int j=0; j<149; j++){
				aux=(reticulo[(j+146)%149]*64+reticulo[(j+147)%149]*32+reticulo[(j+148)%149]*16+reticulo[j%149]*8+reticulo[(j+1)%149]*4+reticulo[(j+2)%149]*2+reticulo[(j+3)%149]*1);	
				evol[j]=id.regra[aux];
			}
			for ( int r=0; r<149; r++){
				reticulo[r]=evol[r];
...
individuo avalia_r3_final (individuo id){
	int reticulo[149], evol[149], notas=0, aux, zeros;	
	for( int i=0; i<10000; i++){
		for ( int ri=0; ri<149; ri++){
				reticulo[ri]=rand()%2;
...
individuo avalia_r3_final_final (individuo id){
	int reticulo[149], evol[149], notas=0, aux, zeros;	
...
int main (){
	...
	for ( int mod=0; mod<100; mod++){	
		gera_reti();		
		for (  i=0; i<100; i++){
			pop[i]=gera_ind(pop[i],i%(tama+1),tama); // pra virar aleatorio eh so substituir i%(tama+1) por uma função rand 
		...
		pop[0]=avalia_r2_final_final(pop[0]);
		printf("%lf\n", pop[0].nota );	
	}
